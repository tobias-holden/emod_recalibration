...
class AnnualSummaryReportAnalyzer(BaseAnalyzer):
    ...
    def reduce(self, all_data: Dict[Union[IWorkflowItem, Simulation], Any]) -> Any:
        ...
        df_summarized = df_final.groupby(['Age']+groupby_tags)['Prevalence', 'Incidence', 'Population'].apply(np.mean).reset_index()
        df_summarized_std = df_final.groupby(['Age']+groupby_tags)['Prevalence', 'Incidence', 'Population'].apply(np.std)
